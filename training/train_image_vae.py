import torch
import torch.nn as nn
import torch.optim as optim
from torch.utils.data import DataLoader
from torch.optim.lr_scheduler import ReduceLROnPlateau
from torchvision.models import vgg16
from models.autoencoder.image_vae import ImageVAE
from datasets import ImageDataset
import os
import math

# Paths
IMAGE_PATH = "datasets/abstract_art/"
IMAGE_MODEL_PATH = "tmodels/image_vae.pth"

# Hyperparameters
BATCH_SIZE = 16
EPOCHS = 75
LEARNING_RATE = 1e-4
LATENT_DIM = 64
MAX_BETA = 3.0  # Reduced from 10.0
RECON_WEIGHT = 1.0  # Increased from 0.05
PERCEPTUAL_LOSS_WEIGHT = 0.1  # Increased from 0.01
WARMUP_EPOCHS = 5
KLD_LOWER_BOUND = 3.0
KLD_UPPER_BOUND = 6.0
BETA_ADJUST_RATE = 0.5
MIN_KLD = 3.0
CYCLE_LENGTH = 5  # Reduced from 10

# Device
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# Dataset and Loader
image_dataset = ImageDataset(IMAGE_PATH, train=True)
image_loader = DataLoader(image_dataset, batch_size=BATCH_SIZE, shuffle=True)

# Perceptual Loss (VGG16)
vgg = vgg16(pretrained=True).features[:16].to(device).eval()
for param in vgg.parameters():
    param.requires_grad = False

# Model with weight initialization
image_vae = ImageVAE(latent_dim=LATENT_DIM).to(device)

# Initialize weights
def initialize_weights(m):
    if isinstance(m, (nn.Conv2d, nn.ConvTranspose2d, nn.Linear)):
        torch.nn.init.xavier_uniform_(m.weight, gain=nn.init.calculate_gain('relu'))
        if m.bias is not None:
            torch.nn.init.zeros_(m.bias)

image_vae.apply(initialize_weights)

# Optimizer and Scheduler
image_optimizer = optim.Adam(image_vae.parameters(), lr=LEARNING_RATE, weight_decay=1e-5)
image_scheduler = ReduceLROnPlateau(image_optimizer, mode='min', factor=0.5, patience=30)

# Cyclical annealing for KLD weight
def cyclical_annealing(epoch, cycle_length=CYCLE_LENGTH):
    cycle = math.floor(1 + epoch / (2 * cycle_length))
    x = abs(epoch / cycle_length - 2 * cycle + 1)
    tau = max(0, 1 - x)
    return tau

# Perceptual loss using VGG16
def perceptual_loss(x, recon_x):
    x_vgg = vgg(x)
    recon_vgg = vgg(recon_x)
    return nn.MSELoss()(x_vgg, recon_vgg)

# Loss function with cyclical annealing and minimum KLD constraint
def vae_loss(recon_x, x, mu, logvar, beta, epoch):
    MSE = nn.MSELoss()(recon_x, x)
    percep_loss = perceptual_loss(recon_x, x)
    logvar = torch.clamp(logvar, min=-10, max=10)
    KLD = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
    kld_weight = cyclical_annealing(epoch)
    kld_loss = beta * kld_weight * KLD
    kld_penalty = 0.0
    if KLD < MIN_KLD:
        kld_penalty = (MIN_KLD - KLD) * 10.0
    return RECON_WEIGHT * MSE + kld_loss + PERCEPTUAL_LOSS_WEIGHT * percep_loss + kld_penalty, MSE, KLD

# Linear warmup for beta
def warmup_beta(epoch, warmup_epochs=WARMUP_EPOCHS, max_beta=MAX_BETA, min_beta=0.001):
    if epoch < warmup_epochs:
        return min_beta + (max_beta - min_beta) * (epoch / warmup_epochs)
    return max_beta

# Dynamic beta adjustment based on KLD
def adjust_beta(current_beta, kld, lower_bound=KLD_LOWER_BOUND, upper_bound=KLD_UPPER_BOUND, adjust_rate=BETA_ADJUST_RATE):
    if kld < lower_bound:
        return min(MAX_BETA, current_beta * (1 + adjust_rate))
    elif kld > upper_bound:
        return max(0.001, current_beta * (1 - adjust_rate))
    else:
        return current_beta

# Training Image VAE
print("Training Image VAE...")
current_beta = 0.001
for epoch in range(EPOCHS):
    image_vae.train()
    image_loss = 0
    mse_loss_total = 0
    kld_loss_total = 0
    base_beta = warmup_beta(epoch)
    for images in image_loader:
        images = images.to(device)
        image_optimizer.zero_grad()
        mu, logvar, skips = image_vae.encode(images)
        z = image_vae.reparameterize(mu, logvar)
        recon = image_vae.decode(z, skips)
        loss, mse, kld = vae_loss(recon, images, mu, logvar, current_beta, epoch)
        loss.backward()
        torch.nn.utils.clip_grad_norm_(image_vae.parameters(), max_norm=10.0)
        image_optimizer.step()
        image_loss += loss.item()
        mse_loss_total += mse.item()
        kld_loss_total += kld.item()
    avg_kld = kld_loss_total / len(image_loader)
    current_beta = adjust_beta(base_beta, avg_kld)
    image_scheduler.step(image_loss / len(image_loader))
    print(f"Epoch [{epoch+1}/{EPOCHS}], Image Loss: {image_loss/len(image_loader):.4f}, MSE: {mse_loss_total/len(image_loader):.4f}, KLD: {kld_loss_total/len(image_loader):.4f}, Beta: {current_beta:.4f}")

# Save the model
torch.save(image_vae.state_dict(), IMAGE_MODEL_PATH)
print(f"✅ Image VAE model saved to {IMAGE_MODEL_PATH}")